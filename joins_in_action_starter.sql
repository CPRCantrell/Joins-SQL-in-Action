...
select * from movies
	left join users on movie_id = favorite_movie_id;
...
select * from movies
	right join users on movie_id = favorite_movie_id;
...
select * from movies
	Inner join users on movie_id = favorite_movie_id;

-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 4 >>>>>>>>>>>>>>>>>>>>>>>
-- Determine which lead studio's movies are favorited by users the most.
-- EXPECTED RESULT: Disney

select lead_studio from
(
	select lead_studio, count(favorite_movie_id) as fan_count from movies
		Inner Join users on movie_id = favorite_movie_id
...
select film from
(
	select film, count(film) as title_count from movies
    group by film
